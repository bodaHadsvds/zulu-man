<template>
    ...
    <v-row>
      ...
      <v-col cols="6" class="d-flex align-center justify-end">
        <v-btn color="primary" variant="flat" @click="dialog = true">
          {{ $vuetify.locale.t("$vuetify.global.lead.types.status.add") }}
        </v-btn>
        <add-edit-dialog
          :dialog="dialog"
          @close="closeDialog"
  ...
  >
    ...
    <template
      v-if="this.$store.state.leadsSettings.statusDataSuccess == false"
      v-slot:no-data
    >
      <div class="py-5">
        <svg
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getLeads() {
        return this.$store.getters["leadsSettings/leadsStatus"].data;
      },
      leadsTotal() {
        const leads = this.$store.getters["leadsSettings/leadsStatus"].total;
