<template>
  <v-toolbar class="bg-background px-0 py-4 pt-0">
    <v-row>
      <v-col cols="6" class="d-flex align-center justify-start">
        <h2>
          {{ $vuetify.locale.t("$vuetify.global.lead.types.reason.plural") }}
        </h2>
      </v-col>
      <v-col cols="6" class="d-flex align-center justify-end">
        <v-btn color="primary" variant="flat" @click="dialog = true">
          {{ $vuetify.locale.t("$vuetify.global.lead.types.reason.add") }}
        </v-btn>
        <add-edit-dialog
          :dialog="dialog"
          @close="closeDialog"
  ...
  <v-data-table-server
    ...
    :headers="headers"
    :loading="this.$store.state.leadsSettings.reasonsLoading"
    :items="leads"
    item-value="id"
    class="elevation-3"
  ...
  >
    ...
    <template
      v-if="this.$store.state.leadsSettings.reasonsDataSuccess == false"
      v-slot:no-data
    >
      <div class="py-5">
        <svg
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getLeads() {
        return this.$store.getters["leadsSettings/leadsReasons"].data;
      },
      leadsTotal() {
        const leads = this.$store.getters["leadsSettings/leadsReasons"].total;
