<template>
  <v-data-table-server
    ...
    :headers="headers"
    :loading="this.$store.state.sessions.bookingsLoading"
    :items="bookings"
    :items-per-page="10"
    item-value="id"
  ...
  >
    ...
    <template
      v-if="this.$store.state.sessions.bookingsDataSuccess == false"
      v-slot:no-data
    >
      <div class="py-5">
        <svg
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      getBookings() {
        const prods = this.$store.getters["sessions/bookings"].data;
      ...
      bookingsTotal() {
        const sales = this.$store.getters["sessions/bookings"].total;
        return parseInt(sales);
      },
    },
    methods: {
      loadBookings() {
        this.$store.dispatch("sessions/fetchBookings", {
